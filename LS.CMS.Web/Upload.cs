...
namespace LS.CMS.Web
{
    public class UpLoad
    {
        private ls_sysconfig sysConfig;
        ...
        public string FileSaveAs(byte[] byteData, string fileName, bool isThumbnail, bool isWater)
        {
            try
            {
                ...
                if (IsImage(fileExt) && (this.sysConfig.img_max_height > 0 || this.sysConfig.img_max_width > 0))
                {
                    byteData = Thumbnail.MakeThumbnailImage(byteData, fileExt, this.sysConfig.img_max_width, this.sysConfig.img_max_height);
                }
                //如果是图片，检查是否需要生成缩略图，是则生成
                if (IsImage(fileExt) && isThumbnail && this.sysConfig.thumbnail_width > 0 && this.sysConfig.thumbnail_height > 0)
                {
                    thumbData = Thumbnail.MakeThumbnailImage(byteData, fileExt, this.sysConfig.thumbnail_width, this.sysConfig.thumbnail_height, this.sysConfig.thubnail_mode);
                }
                else
                {
                ...
                {
                    switch (this.sysConfig.watermark_type)
                    {
                        case 1:
                            byteData = WaterMark.AddImageSignText(byteData, fileExt, this.sysConfig.watermark_text, this.sysConfig.watermark_position,
                                this.sysConfig.watermark_img_quality, this.sysConfig.watermark_font, this.sysConfig.watermark_fontsize);
                            break;
                        case 2:
                            byteData = WaterMark.AddImageSignPic(byteData, fileExt, this.sysConfig.watermark_pic, this.sysConfig.watermark_position,
                                this.sysConfig.watermark_img_quality, this.sysConfig.watermark_transparency);
                            break;
                    }
                }
        ...
        {
            //文件不应是上传文件，防止跨目录删除
            if (fileUri.IndexOf("..") == -1 && fileUri.ToLower().StartsWith(sysConfig.file_path.ToLower()))
            {
        ...
        {
            string path = sysConfig.file_path + "/"; //站点目录+上传目录
            switch (this.sysConfig.file_save)
            {
                case 1: //按年月日每天一个文件夹
        ...
        {
            //判断是否开启水印
            if (this.sysConfig.watermark_type > 0)
            {
                //判断是否可以打水印的图片类型
        ...
        private bool CheckFileExt(string _fileExt)
        {
            ...
            //检查合法文件
            string[] allowExt = (this.sysConfig.file_extension + "," + this.sysConfig.video_extension).Split(',');
        ...
        {
            //将视频扩展名转换成ArrayList
            ArrayList lsVideoExt = new ArrayList(this.sysConfig.video_extension.ToLower().Split(','));
            //判断是否为图片文件
            if (IsImage(_fileExt))
            {
                if (this.sysConfig.img_size > 0 && _fileSize > this.sysConfig.img_size * 1024)
                {
                    return false;
                }
            }
            ...
            {
                if (this.sysConfig.video_size > 0 && _fileSize > this.sysConfig.video_size * 1024)
                {
                    return false;
                }
            }
            ...
            {
                if (this.sysConfig.attach_size > 0 && _fileSize > this.sysConfig.attach_size * 1024)
                {
                    return false;
                }
            }
